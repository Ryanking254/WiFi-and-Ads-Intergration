...
app.delete('/api/campaigns/:id', authMiddleware, (req, res) => {
  try {
    db.prepare('DELETE FROM campaigns WHERE id = ? AND advertiser_id = ?')
      .run(req.params.id, req.advertiserId);
...
app.get('/api/campaigns/:id/analytics', authMiddleware, (req, res) => {
  ...
  try {
    const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ? AND advertiser_id = ?')
      .get(req.params.id, req.advertiserId);
...
app.get('/api/advertisers/:id', (req, res) => {
  try {
    const advertiser = db.prepare('SELECT id, name, email, company, daily_budget FROM advertisers WHERE id = ?')
      .get(req.params.id);
